#---------------------------------------------------------------------------
# 構築：プロンプト：試験：gpt

import re
import json

class ConLog:

  @classmethod
  def catsys(self, diccnf):
    txtsys = diccnf["status"]
    self.maxlog = diccnf["maxlog"] + 2
    self.lstsys = []
    self.lstlog = []
    dicsys = {"role": "system", "content": txtsys}
    self.lstsys.append(dicsys)

  @classmethod
  def catreq(self, txtreq):
    dicreq = json.loads(txtreq)
    dicopt = {"role": "system", "content": dicreq["txtopt"]} # 補足の情報
    dicusr = {"role": "user", "content": dicreq["txtusr"]}
    self.lstlog.append(dicusr)
    lstprm = []
    # 補足の情報を、２コ目のシステムプロンプトとして挿入
    lsttmp = self.lstsys.copy()
    lsttmp.append(dicopt)
    lstprm.extend(lsttmp)
    #
    lstprm.extend(self.lstlog)
    return lstprm

  @classmethod
  def catres(self, txtres):
    dicagt = {"role": "assistant", "content": txtres}
    self.lstlog.append(dicagt)
    if len(self.lstlog) > self.maxlog:
      self.lstlog.pop(0)
      self.lstlog.pop(0)

  @classmethod
  def arrres(self, txtifr):
    txtres = txtifr
    return (txtres, "")